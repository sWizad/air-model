...
def encoder(inputs, input_dim, rec_hidden_units, latent_dim, activation=tf.nn.softplus):
...
def decoder(inputs, input_dim, gen_hidden_units, latent_dim, activation=tf.nn.softplus):
    input_size = tf.shape(inputs)[0]
    likelihood_std = 0.3
    next_layer = inputs
    for i in range(len(gen_hidden_units)):
    ...
    with tf.variable_scope("gen_mean") as scope:
        generative_mean = layers.fully_connected(next_layer, input_dim, activation_fn=None, scope=scope)
        #generative_mean = tf.nn.sigmoid(genative_mean)
