...
def main(arv):
    ...
    def enc_block(input,filter=32,kernel_size = 3,block=''):
        with slim.arg_scope(
            ...
            activation_fn=lambda x: tf.nn.leaky_relu(x, alpha=0.1),
            normalizer_params={"is_training": is_train}):
            conv1 = slim.conv2d(input, filter, [3, 3], scope=block+"conv%d" % (1))
            conv2 = slim.conv2d(conv1, filter, [3, 3], scope=block+"conv%d" % (2))
            drop1 = slim.dropout(conv2, 0.5, scope=block+"dropout")
            layer = slim.max_pool2d(drop1, [2, 2], padding='SAME')
        return drop1,layer

    def dec_block(input,bypart,w,filter=32,kernel_size = 3,block=''):
        with slim.arg_scope(
            [slim.conv2d, slim.fully_connected],
            normalizer_fn=slim.batch_norm,
            activation_fn=lambda x: tf.nn.leaky_relu(x, alpha=0.1),
            normalizer_params={"is_training": is_train}):
            upsam = tf.image.resize_images(input,(w,w))
            conv1 = slim.conv2d(upsam, filter, [3, 3], scope=block+"deconv%d" % (1))
            merge = tf.concat([conv1, bypart],axis=3)
            conv2 = slim.conv2d(merge, filter, [3, 3], scope=block+"deconv%d" % (2))
            conv3 = slim.conv2d(conv2, filter, [3, 3], scope=block+"deconv%d" % (3))
            layer = slim.dropout(conv3, 0.75, scope=block+"dropout")
        return layer

    def unet(input,T):
        unet_hidden_units = (16,32,64)
        num_step = len(unet_hidden_units)
        with tf.variable_scope("unet"):
            layer = [0]*num_step#tf.concat([input, x_hat],axis=3)
            #layer = input if i==0 else net[i-1]
            net = input
            with tf.variable_scope("encode"):
                for i in range(num_step):
                    layer[i], net  = enc_block(net,unet_hidden_units[i],3,block='%d_'%(i))

            with tf.variable_scope("joint"):
                conv1 = slim.conv2d(net, 128, [3, 3], scope="join_conv%d" % (1))
                conv2 = slim.conv2d(conv1, 128, [3, 3],normalizer_fn=slim.batch_norm, scope="join_conv%d" % (2))
                net = slim.dropout(conv2, 0.75, scope="join_dropout")
                w = 7
            ...
            with tf.variable_scope("decode"):
                for i in range(num_step):
                    w *=2
                    j = num_step-i-1
                    net = dec_block(net,layer[j],w,unet_hidden_units[j],3,block='%d_'%(i))
